...
api.getId = function(actor, id, callback) {
  async.auto({
    ...
    update: ['find', function(callback, results) {
      ...
      userSettings.update(q, u, {upsert: true}, function(err) {
...
bedrock.events.on('bedrock.init', function(callback) {
  ...
  scheduler.define('notifications.jobs.immediate', function(job, callback) {
    async.each(registeredModules, function(module, callback) {
      ...
      module.process(processOptions, function(err) {
...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
  ...
  app.post(
    ...
    ensureAuthenticated, function(req, res, next) {
      ...
      updateSettings(req.user.identity, req.body, function(err) {
        if(err) {
          return next(err);
        }
        res.sendStatus(204);
...
// Used in unit testing, but we can probably take advantage of this
// functionality somewhere else in the future.
